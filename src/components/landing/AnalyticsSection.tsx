...
export default function AnalyticsSection() {
  return (
    <section aria-label="Academic analytics" className="relative w-full bg-white border-t border-slate-200/60 overflow-hidden">
      <div className="max-w-[1280px] mx-auto px-5 sm:px-8 lg:px-12 py-14 lg:py-20">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          ...
          <motion.div variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true }}
            className="rounded-2xl border border-slate-200/80 bg-white p-6 sm:p-7 hover:shadow-[0_12px_30px_rgba(15,23,42,0.08)] transition-all duration-300">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-[15px] font-bold text-[#0F172A]">CGPA Growth Trend</h3>
                  <p className="text-[11px] text-slate-500 font-medium">Semester-over-semester</p>
                </div>
              </div>
              <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-emerald-50 border border-emerald-100">
                <TrendingUp size={12} className="text-emerald-600" />
                <span className="text-[11px] font-bold text-emerald-700">+12.4%</span>
              </div>
            </div>
            <MiniLineChart />
            <div className="flex items-center justify-between mt-4 pt-4 border-t border-slate-100">
              {['Sem 1: 7.2', 'Sem 3: 8.1', 'Sem 6: 9.3'].map((s) => (
                <span key={s} className="text-[11px] font-semibold text-slate-500">{s}</span>
              ))}
            </div>
          </motion.div>
          ...
          <motion.div variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true }}
            ...
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h3 className="text-[15px] font-bold text-[#0F172A]">Academic Progress</h3>
                <p className="text-[11px] text-slate-500 font-medium">Overall completion metrics</p>
              </div>
            </div>
            <div className="flex items-center justify-around">
              <CircularProgress value={87} label="CGPA Target" color="#2563EB" />
              <CircularProgress value={79} label="Credits Done" color="#4F46E5" />
          ...
          <motion.div variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true }}
            ...
            {[
              { icon: Zap, title: 'Strongest Subject', value: 'Data Structures', sub: 'SGPA: 9.8', iconColor: 'text-amber-600', bg: 'bg-amber-50 border-amber-100' },
              { icon: BarChart3, title: 'Average SGPA', value: '8.72', sub: 'Across 6 semesters', iconColor: 'text-blue-600', bg: 'bg-blue-50 border-blue-100' },
              { icon: Target, title: 'Predicted Final CGPA', value: '8.9 - 9.1', sub: 'Based on current trend', iconColor: 'text-purple-600', bg: 'bg-purple-50 border-purple-100' },
            ].map((c) => (
              <div key={c.title} className="rounded-xl border border-slate-200/80 bg-white p-5 shadow-[0_2px_8px_rgba(15,23,42,0.02)] hover:shadow-[0_8px_24px_rgba(15,23,42,0.06)] transition-all duration-300">
                ...
                <div className="text-xl font-extrabold text-[#0F172A] mb-0.5">{c.value}</div>
                <span className="text-[11px] font-semibold text-slate-500">{c.sub}</span>
              </div>
            ))}
          </motion.div>
        </div>
